...
export default function PrescriptionsScreen() {
    ...
    const fetchPrescriptions = async () => {
      try {
        ...
        const records = snapshot.docs
          ...
          .filter((record) => record.category === 'prescription'); // Filter for Prescription records

        setPrescriptions(records);
      } catch (error) {
        console.error('Error fetching prescription records:', error);
        Alert.alert('Error', 'Could not fetch prescription records.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handlePressPrescription = (imageUri) => {
    setSelectedImage(imageUri);
    setModalVisible(true);
  };

  const renderPrescription = ({ item }) => (
    <TouchableOpacity style={styles.card} onPress={() => handlePressPrescription(item.imageUri)}>
      <Image source={{ uri: item.imageUri }} style={styles.image} />
      <View style={styles.details}>
        <Text style={styles.title}>{item.medication || 'Unnamed Prescription'}</Text>
